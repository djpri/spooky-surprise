...
export default function MenuAudioLoop() {
  ...
  // If there's custom scene audio, don't play background music
  const activeTrack = sceneAudio ? null : currentPath || "menu";
  ...
  useEffect(() => {
    ...
    // If sound is disabled or scene has custom audio, fade out any playing nodes and exit.
    if (!soundEnabled || !activeTrack) {
      gainNodesRef.current.forEach((g) => {
        g.gain.linearRampToValueAtTime(
          0,
      ...
      currentTrackRef.current = null; // Reset track reference

      // Schedule a complete stop after fade completes
      ...
      }, 350); // Slightly longer than fade time to ensure clean stop
    ...
    const gainNodes: GainNode[] = [context.createGain(), context.createGain()];
    ...
    const playSlot = (
      ...
      const fadeOutStart = Math.max(fadeInEnd, stopTime - FADE_OUT_SECONDS);
      ...
      const delayMs = Math.max((nextStart - context.currentTime) * 1000, 0);
      scheduleTimeout(
  ...
  }, [soundEnabled, activeTrack, sceneAudio, kick]);
